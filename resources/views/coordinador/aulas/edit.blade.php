...
<div class="bg-white rounded-2xl border p-6 max-w-xl">
  <h1 class="text-xl font-semibold mb-4">Editar aula</h1>
  ...
  <form method="POST" action="{{ route('coordinador.aulas.update', $aula) }}" class="space-y-4">
    ...
    <div>
      ...
      <input type="number" name="piso" value="{{ old('piso', $aula->piso) }}" min="0" max="100" required
    ...
    <div>
      ...
      <input type="text" name="numero" value="{{ old('numero', $aula->numero) }}" maxlength="50" required
             class="mt-1 w-full rounded-xl bg-slate-50 ring-1 ring-slate-200 focus:ring-2 focus:ring-emerald-500 px-4 py-3">
    </div>
    <div>
      <label class="text-sm text-slate-700">Capacidad</label>
      <input type="number" name="capacidad" value="{{ old('capacidad', $aula->capacidad) }}" min="1" max="500" required
             class="mt-1 w-full rounded-xl bg-slate-50 ring-1 ring-slate-200 focus:ring-2 focus:ring-emerald-500 px-4 py-3">
    </div>
